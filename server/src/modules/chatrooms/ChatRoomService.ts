...
export class ChatRoomService {
  ...
  async addToChatroom(chatroomId: String, friendId: string){
    return this.model.findByIdAndUpdate(chatroomId, {$addToSet: {users: friendId}}, {new: true}).populate("users").exec();
  }

  async removeFromChatroom(chatroomId: String, friendId: string){
    return this.model.findByIdAndUpdate(chatroomId,{$pull: {users: friendId}});
  }
