...
export default class MessageResolver {
  ...
  async addMessage(@Arg('message') message: MessageInput, @Ctx() ctx: Context) {
    const {msg, chatroom, guestId, guestName, avatarUrl} = message;
    const {userId = null, user} = ctx;

      const {socket} = await socketIO();
      return await this.service.addMessage(userId, msg, chatroom, guestId, guestName, avatarUrl)
          .then(({_id: messageId, msg, chatroom, guestId, guestName, createdAt, avatarUrl}) => {
              const nickname = guestName || user.profile.firstName;
              const _userId = guestId || userId;
              socket.to(chatroom).emit("message", {_id: messageId, from: {_id: _userId, nickname, avatarUrl}, msg, createdAt});
              return messageId
          })
