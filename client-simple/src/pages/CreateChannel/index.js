...
class Step1 extends Component {
  ...
  render() {
    const { canGoNext, title, description, onDescriptionChange, nextStep, isPrivate, onCheckboxChange } = this.props;
    const { validTitle } = this.state;
    ...
    return (
      <div className="page__wrapper page__wrapper--absolute steps__wrapper">
        ...
        <Checkbox onChange={onCheckboxChange}>Private Chat</Checkbox>
...
class CreateChannel extends Component {
  ...
  handleFormSubmit = async e => {
    ...
    if (this.canSubmitForm()) {
      const { channelTitle, channelDescription, channelLocation, isPrivate} = this.state;
      const chatroom = {name: channelTitle, description: channelDescription, ...channelLocation, isPrivate};

      if (createChannel) {
        const {data: {createNewChatroom: newChatroomId}} = await createChannel({variables: {chatroom}});
        if (newChatroomId) {
          if(this.state.isPrivate) this.props.history.push(`/pchat/${newChatroomId}`);
          else this.props.history.push(`/chat/${newChatroomId}`);
        }
      }
    }
  };
  ...
  renderCurrentStep = currentStep => {
    const { channelTitle, channelDescription, channelAddress, isPrivate } = this.state;
