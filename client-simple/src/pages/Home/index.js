    ...
    if (chatrooms && chatrooms.length) {
      return mapVisible ? (
        ...
        <div className="chatrooms">
          {chatrooms.map(({ _id, name, users, verified, isPrivate }) => (
            <ChannelItem key={_id} id={_id} title={name} users={users} verified={verified} isPrivate={isPrivate} toggleModal={this.toggleModal} />
          ))}
        </div>
      );
    }
